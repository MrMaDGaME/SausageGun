...
public class shoot_submachine : MonoBehaviourPunCallbacks
{
    ...
    public float magazine_size = 40;

    public float current_magazine;

    public bool can_shoot = true;

    public float reloading_time = 1;
    ...
    void Update()
    {
        ...
        if (Input.GetButton("Fire1") && fireTimer >= fireRate && can_shoot && current_magazine > 0)
        {
            FPS_Camera.GetComponent<AudioSource>().clip = FPS_Camera.GetComponent<Sons>().submachine_sound;
            FPS_Camera.GetComponent<AudioSource>().Play();
            
            Debug.Log("tir de mitraillette");
            ...
            if (Physics.Raycast(ray, out _hit, 500))
            {
                if (_hit.collider.gameObject.CompareTag("PlayerRed") &&!_hit.collider.gameObject.GetComponent<PhotonView>().IsMine)
                {
                    if (PhotonNetwork.LocalPlayer.CustomProperties.ContainsValue("blue"))
                    {
                        ...
                        _hit.collider.gameObject.GetComponent<PhotonView>().RPC("Knockback_sub", RpcTarget.AllBuffered, knockback_vector);
                    }
                }
    ...
    [PunRPC]
    void Knockback_sub(Vector3 knockback_vector)
    {
