...
public class shoot_sniper : MonoBehaviourPunCallbacks
{
    ...
    public float magazine_size = 5;

    public float current_magazine;

    public bool can_shoot = true;

    public float reloading_time = 4;
    ...
    void Update()
    {
        ...
        {
            FPS_Camera.GetComponent<AudioSource>().clip = FPS_Camera.GetComponent<Sons>().sniper_sound;
            FPS_Camera.GetComponent<AudioSource>().Play();

            Debug.Log("tir de sniper");
            ...
            if (Physics.Raycast(ray, out _hit, 500))
            {
                if (_hit.collider.gameObject.CompareTag("PlayerRed") &&!_hit.collider.gameObject.GetComponent<PhotonView>().IsMine)
                {
                    if (PhotonNetwork.LocalPlayer.CustomProperties.ContainsValue("blue"))
                    {
                        ...
                        _hit.collider.gameObject.GetComponent<PhotonView>().RPC("Knockback_sniper", RpcTarget.AllBuffered, knockback_vector);
                    }
                }
    ...
    [PunRPC]
    void Knockback_sniper(Vector3 knockback_vector)
    {
