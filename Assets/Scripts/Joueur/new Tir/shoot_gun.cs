...
public class shoot_gun : MonoBehaviourPunCallbacks
{
    ...
    void Update()
    {
        ...
        {
            FPS_Camera.GetComponent<AudioSource>().clip = FPS_Camera.GetComponent<Sons>().gun_sound;
            FPS_Camera.GetComponent<AudioSource>().Play();

            Debug.Log("tir de flingue");
            ...
            if (Physics.Raycast(ray, out _hit, 500))
            {
                if (_hit.collider.gameObject.CompareTag("PlayerRed") &&!_hit.collider.gameObject.GetComponent<PhotonView>().IsMine)
                {
                    if (PhotonNetwork.LocalPlayer.CustomProperties.ContainsValue("blue"))
                    {
                        ...
                        _hit.collider.gameObject.GetComponent<PhotonView>().RPC("Knockback_gun", RpcTarget.AllBuffered, knockback_vector);
                    }
                }
    ...
    [PunRPC]
    void Knockback_gun(Vector3 knockback_vector)
    {
